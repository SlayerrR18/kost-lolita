<x-guest-layout>
    ...
    <div class="px-4 py-2">
        ...
        <div class="text-center mb-8">
            ...
            <h2 class="text-3xl font-serif font-bold text-[#222831]">Selamat Datang</h2>
            <p class="text-gray-500 text-sm mt-2">Silakan masuk untuk melanjutkan ke dashboard.</p>
        </div>
        ...
        <form method="POST" action="{{ route('login') }}" class="space-y-5">
            ...
            <div class="flex items-center justify-between pt-1">
                <label for="remember_me" class="inline-flex items-center cursor-pointer group">
                    <input id="remember_me" type="checkbox" class="rounded border-gray-300 text-[#222831] shadow-sm focus:ring-[#222831] cursor-pointer" name="remember">
                    <span class="ms-2 text-sm text-gray-600 group-hover:text-gray-900 transition-colors">{{ __('Ingat saya') }}</span>
                </label>

                @if (Route::has('password.request'))
                    <a class="text-sm font-medium text-[#222831] hover:text-[#DFD0B8] hover:underline transition-colors" href="{{ route('password.request') }}">
                        {{ __('Lupa password?') }}
                    </a>
                @endif
            </div>
        ...
        @if (Route::has('register'))
            <div class="mt-8 pt-6 border-t border-gray-100 text-center">
                <p class="text-sm text-gray-500">
                    Belum memiliki akun?
                    <a href="{{ route('register') }}" class="font-bold text-[#222831] hover:text-[#DFD0B8] transition-colors ml-1">
                        Daftar disini
                    </a>
                </p>
            </div>
