<nav x-data="{ open: false, dropdownOpen: false }" class="bg-white border-b border-gray-200 sticky top-0 z-40 shadow-sm font-sans">
            ...
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    <a href="{{ route($dashboardRoute) }}"
                       class="{{ $navClass }} {{ request()->routeIs($dashboardRoute) ? $activeStyle : $inactiveStyle }}">
                        {{ __('Dashboard') }}
                    </a>

                    @if(auth()->user()->role !== 'admin')
                        <a href="#" class="{{ $navClass }} {{ $inactiveStyle }}">
                            Riwayat Sewa
                        </a>
                        <a href="#" class="{{ $navClass }} {{ $inactiveStyle }}">
                            Tagihan
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden bg-white border-t border-gray-100">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <a href="{{ route($dashboardRoute) }}" class="{{ $respClass }} {{ request()->routeIs($dashboardRoute) ? $respActive : $respInactive }}">
                {{ __('Dashboard') }}
            </a>

            @if(auth()->user()->role !== 'admin')
                <a href="#" class="{{ $respClass }} {{ $respInactive }}">
                    Riwayat Sewa
                </a>
            @endif
        </div>
