...
@section('title', 'Laporan Pendapatan - Admin Kost Lolita')
...
<div x-data="{
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            <div>
                <h1 class="text-3xl font-serif font-bold text-[#222831]">Laporan Pendapatan</h1>
                <p class="text-gray-500 text-sm mt-1">
                    Pantau arus kas masuk dari sewa kost dan sumber lainnya.
                </p>
            </div>

            <a href="{{ route('admin.finance.income.create') }}"
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div class="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex items-center justify-between group hover:shadow-md transition-all">
                <div>
                    ...
                    <h3 class="text-2xl font-bold text-green-600 mt-1">Rp {{ number_format($totalIncome, 0, ',', '.') }}</h3>
                </div>
                <div class="w-12 h-12 rounded-2xl bg-green-50 flex items-center justify-center text-green-500 group-hover:scale-110 transition-transform">
            ...
            <div class="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex items-center justify-between group hover:shadow-md transition-all">
                <div>
                    ...
                    <h3 class="text-2xl font-bold text-[#222831] mt-1">{{ $incomes->total() }} <span class="text-sm font-normal text-gray-400">Item</span></h3>
                </div>
            ...
            <div class="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex items-center justify-between group hover:shadow-md transition-all">
                <div>
                    ...
                    <h3 class="text-2xl font-bold text-[#222831] mt-1">
                        Rp {{ $incomes->total() > 0 ? number_format($totalIncome / $incomes->total(), 0, ',', '.') : '0' }}
                    </h3>
                </div>
        ...
        <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-6 mb-8" x-data="{ showFilter: false }">
            ...
            <div x-show="showFilter" x-collapse style="display: none;">
                <form method="GET" action="{{ route('admin.finance.income.index') }}" class="mt-6 pt-6 border-t border-gray-100">
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        <div class="flex items-end gap-2">
                            ...
                            <a href="{{ route('admin.finance.income.index') }}" class="px-4 py-2.5 bg-gray-100 text-gray-500 rounded-xl hover:bg-gray-200 transition-colors" title="Reset Filter">
                ...
                <table class="w-full text-left border-collapse">
                    <thead class="bg-[#222831] text-[#DFD0B8]">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-xs font-bold uppercase tracking-wider">Sumber</th>
                        ...
                        @forelse($incomes as $income)
                            <tr class="hover:bg-gray-50 transition-colors duration-200 group">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="text-sm font-medium text-gray-600">{{ $income->date->format('d M Y') }}</span>
                                </td>

                                <td class="px-6 py-4">
                                    <div class="text-sm font-bold text-[#222831]">{{ $income->source }}</div>
                                </td>

                                <td class="px-6 py-4">
                                    @php
                                        $catColor = match($income->category) {
                                            'sewa_kost' => 'bg-green-100 text-green-700',
                                            'denda' => 'bg-red-100 text-red-700',
                                ...
                                <td class="px-6 py-4 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        <a href="{{ route('admin.finance.income.show', $income) }}"
                                        ...
                                        <button @click="deleteModalOpen = true; deleteAction = '{{ route('admin.finance.income.destroy', $income) }}'; incomeSource = '{{ $income->source }}'"
                        ...
                        <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                            ...
                            <div class="mt-2">
                                <p class="text-sm text-gray-500">
                                    Anda yakin ingin menghapus data dari <strong><span x-text="incomeSource"></span></strong>? <br>
