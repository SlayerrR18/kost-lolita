...
<div class="py-8" x-data="expenseEditForm({{ $expense->amount }})">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            ...
            <h1 class="text-3xl font-serif font-bold text-[#222831]">Edit Data Pengeluaran</h1>
            <p class="text-gray-500 mt-1">Perbarui informasi biaya #{{ $expense->id }}</p>
        </div>
        ...
        <div class="bg-white rounded-3xl shadow-lg border border-gray-100 overflow-hidden relative">
            ...
            <form action="{{ route('admin.finance.expense.update', $expense) }}" method="POST" enctype="multipart/form-data" class="p-8" @submit="prepareSubmit">
                @csrf
                @method('PATCH')
                ...
                <div class="mb-8 bg-red-50 rounded-2xl p-6 border border-red-100">
                    ...
                    @error('amount') <p class="text-red-600 text-xs mt-2 font-medium">{{ $message }}</p> @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="name" value="{{ old('name', $expense->name) }}" placeholder="Contoh: Beli Token Listrik"
                    ...
                    <div>
                        ...
                        <input type="text" name="reference" value="{{ old('reference', $expense->reference) }}" placeholder="Contoh: INV-001"
                               class="w-full rounded-xl border-gray-300 focus:border-[#222831] focus:ring focus:ring-[#222831]/20 transition py-3 px-4">
                    </div>
                </div>
                ...
                <div class="border-t border-dashed border-gray-200 pt-6 mt-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="border-2 border-dashed border-gray-300 rounded-xl p-1 transition-colors hover:border-[#222831] hover:bg-gray-50"
                                ...
                                <div class="relative cursor-pointer h-40 flex flex-col items-center justify-center rounded-lg overflow-hidden bg-gray-50"
                                    ...
                                    <div x-show="!filePreview && !existingFile" class="text-center p-4">
                                        ...
                                        <p class="text-xs font-bold text-gray-600">Ganti Nota</p>
                                    </div>

                                    @if($expense->bukti_transfer)
                                        <div x-show="!filePreview" class="w-full h-full relative group">
                                            @if(Str::endsWith($expense->bukti_transfer, ['.jpg', '.jpeg', '.png']))
                                                <img src="{{ asset('storage/' . $expense->bukti_transfer) }}" class="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition">
                                            @else
                        ...
                        <div>
                            ...
                            <textarea name="description" rows="5" class="w-full rounded-xl border-gray-300 focus:border-[#222831] focus:ring focus:ring-[#222831]/20 transition p-3 resize-none" placeholder="Keterangan tambahan...">{{ old('description', $expense->description) }}</textarea>
                        </div>
                    </div>
                </div>
...
<script>
    function expenseEditForm(initialAmount) {
        return {
            formattedAmount: '',
            ...
            fileType: '',
            isDragging: false,
            existingFile: {{ $expense->bukti_transfer ? 'true' : 'false' }},
